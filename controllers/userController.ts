...
exports.modifyUserInfo = async (req: Request, res: Response) => {
  ...
  if (!username || !currentPassword) {
    return res.status(400).json({ message: 'username과 currentPassword는 필수 항목입니다.' });
  }

  try {
    // 1. 사용자 정보 조회 (기존 비밀번호 확인)
    const [user]: any = await pool.query(`SELECT password FROM Users WHERE user_id = ?`, [userId]);
    ...
    if (results.affectedRows === 0) {
      return res.status(404).json({ message: '사용자 정보를 업데이트하지 못했습니다.' });
    }
