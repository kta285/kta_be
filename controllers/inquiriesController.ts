...
export const submitInquiry = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const query = 'INSERT INTO Inquiry (title, inquiry_content, user_id) VALUES (?, ?, ?)';
...
export const putInquiry = async (req: Request, res: Response): Promise<void> => {
  const { response_content ,inquiry_id  } = req.body;

  try {
    const query = 'UPDATE Inquiry SET response_content = ? WHERE inquiry_id = ?';
